...
## 你可以做的事
- 呼叫工具查詢即時時刻表（OD 班次）
- 呼叫工具查詢列車車種與編組摘要（含機務段、運用碼、山/海線、ㄏㄙ 回送標記、機務派車時刻、行駛日）
- 呼叫工具推算列車「目前位置」（輸入車次）
- 回答台鐵一般知識（例如「105 是哪個機務段的車」「哪些車屬於七堵機務段」）
...
class GemmaAIService:
    ...
    def _tool_query_consist(self, inp: dict) -> str:
        ...
        if not consist:
            return f"查無 {train_no} 次編組資料。"

        lines = [f"{train_no} 次"]
        level = consist.get("train_level", "")
        type_name = consist.get("type_name", "")
        if level or type_name:
            lines.append(f"車種：{type_name}" + (f"（{level}）" if level and level != type_name else ""))
        if consist.get("formation"):
            lines.append(f"編組：{consist['formation']}")
        if consist.get("route"):
            lines.append(f"營運區間：{consist['route']}")

        depot = consist.get("depot", "")
        unit_code = consist.get("unit_code", "")
        run_type = consist.get("run_type", "")
        depot_parts = [x for x in (depot, f"運用 {unit_code}" if unit_code else "", run_type) if x]
        if depot_parts:
            lines.append(f"機務：{' / '.join(depot_parts)}")

        flags = [f for f in (consist.get("flags") or []) if f != "ㄏㄙ"]
        extras = [f"{f}線" if f in ("山", "海") else f for f in flags]
        if consist.get("is_deadhead"):
            extras.insert(0, "ㄏㄙ 回送車")
        if extras:
            lines.append(f"標記：{' / '.join(extras)}")

        dep = consist.get("dep_time", "")
        arr = consist.get("arr_time", "")
        if dep or arr:
            lines.append(f"機務派車時刻：{dep or '—'} → {arr or '—'}")

        day_cond = consist.get("day_conditions", "")
        if day_cond:
            lines.append(f"行駛日：{day_cond}")
